...
// Headers
////////////////////////////////////////////////////////////
#include <SFML3D/Graphics/Shader.hpp>
#include <SFML3D/Graphics/Texture.hpp>
#include <SFML3D/Graphics/VertexBuffer.hpp>
#include <SFML3D/Graphics/GLCheck.hpp>
#include <SFML3D/System/InputStream.hpp>
...
namespace
{
    ...
    // is used for id
    sf3d::Uint64 getUniqueId()
    {
        static sf3d::Uint64 id = 1; // start at 1, zero is "no program"
        static sf3d::Mutex mutex;
...
namespace
{
    ...
    bool getStreamContents(sf3d::InputStream& stream, std::vector<char>& buffer)
    {
        bool success = true;
        sf3d::Int64 size = stream.getSize();
        if (size > 0)
        {
            buffer.resize(static_cast<std::size_t>(size));
            stream.seek(0);
            sf3d::Int64 read = stream.read(&buffer[0], size);
            success = (read == size);
        }
        buffer.push_back('\0');
        return success;
...
void Shader::setParameter(const std::string& name, int x) const
{
    if (m_shaderProgram)
    {
        ...
        {
            // Enable program
            program = glGetHandle(GL_PROGRAM_OBJECT_EXT);
...
////////////////////////////////////////////////////////////
void Shader::setParameter(const std::string& name, const sf3d::Transform& transform) const
{
    if (m_shaderProgram)
    {
        ensureGlContext();
...
void Shader::beginParameterBlock() const
{
    ...
    m_blockProgram = static_cast<unsigned int>(glGetHandle(GL_PROGRAM_OBJECT_EXT));
...
void Shader::endParameterBlock() const
{
    ...
    if (m_blockProgram != m_shaderProgram)
        glCheck(glUseProgram(m_blockProgram));
}
...
bool Shader::isAvailable()
{
    ...
    return GLEW_shading_language_100 &&
           GLEW_shader_objects       &&
           GLEW_vertex_shader        &&
           GLEW_fragment_shader;
}
...
bool Shader::isUniformBufferAvailable()
{
    ...
    if (!checked)
    {
        ...
        uniformBufferSupported = isAvailable() && VertexBuffer::isAvailable() && GLEW_uniform_buffer_object;
    }
...
std::string Shader::getSupportedVersion()
{
    ...
    if (!checked)
    {
        ...
        if (isAvailable())
        {
            ...
            glCheck(version = glGetString(GL_SHADING_LANGUAGE_VERSION));
...
unsigned int Shader::getMaximumUniformComponents()
{
    ...
    glCheck(glGetIntegerv(GL_MAX_VERTEX_UNIFORM_COMPONENTS, &maxVertexUniformComponents));
...
bool Shader::compile(const char* vertexShaderCode, const char* fragmentShaderCode, const char* geometryShaderCode)
{
    ...
    if (vertexShaderCode)
    {
        ...
        //glCheck(glGetObjectParameteriv(vertexShader, GL_COMPILE_STATUS, &success));
        if (success == GL_FALSE)
        {
            char log[1024];
            /*glCheck(glGetInfoLog(vertexShader, sizeof(log), 0, log));
    ...
    if (fragmentShaderCode)
    {
        ...
        // Check the compile log
        GLint success = GL_TRUE;
        //glCheck(glGetObjectParameteriv(fragmentShader, GL_COMPILE_STATUS, &success));
        if (success == GL_FALSE)
        {
            char log[1024];
            /*glCheck(glGetInfoLog(fragmentShader, sizeof(log), 0, log));
    ...
    // Check the link log
    GLint success = GL_TRUE;
    //glCheck(glGetObjectParameteriv(m_shaderProgram, GL_LINK_STATUS, &success));
    if (success == GL_FALSE)
    {
        char log[1024];
        /*glCheck(glGetInfoLog(m_shaderProgram, sizeof(log), 0, log));
