...
// Headers
////////////////////////////////////////////////////////////
#include <SFML3D/Window/iOS/SFAppDelegate.hpp>
#include <SFML3D/Window/iOS/SFMain.hpp>
#include <vector>
...
namespace
{
    ...
    std::vector<sf3d::Vector2i> touchPositions;
}
...
- (void)applicationWillResignActive:(UIApplication *)application
{
    ...
    {
        sf3d::Event event;
        event.type = sf3d::Event::LostFocus;
...
- (void)applicationDidBecomeActive:(UIApplication *)application
{
    ...
    {
        sf3d::Event event;
        event.type = sf3d::Event::GainedFocus;
        sfWindow->forwardEvent(event);
    }
}
...
{
    sf3d::Vector2u size = self.sfWindow->getSize();
...
////////////////////////////////////////////////////////////
- (sf3d::Vector2i)getTouchPosition:(unsigned int)index
{
    if (index < touchPositions.size())
        return touchPositions[index];
    else
        return sf3d::Vector2i(-1, -1);
}


////////////////////////////////////////////////////////////
- (void)notifyTouchBegin:(unsigned int)index atPosition:(sf3d::Vector2i)position;
{
    ...
    if (index >= touchPositions.size())
        touchPositions.resize(index + 1, sf3d::Vector2i(-1, -1));
    ...
    {
        sf3d::Event event;
        event.type = sf3d::Event::TouchBegan;
...
////////////////////////////////////////////////////////////
- (void)notifyTouchMove:(unsigned int)index atPosition:(sf3d::Vector2i)position;
{
    position.x *= backingScaleFactor;
    position.y *= backingScaleFactor;
    ...
    {
        sf3d::Event event;
        event.type = sf3d::Event::TouchMoved;
        event.touch.finger = index;
        event.touch.x = position.x;
        event.touch.y = position.y;
        sfWindow->forwardEvent(event);
...
////////////////////////////////////////////////////////////
- (void)notifyTouchEnd:(unsigned int)index atPosition:(sf3d::Vector2i)position;
{
    // clear the touch position
    if (index < touchPositions.size())
        touchPositions[index] = sf3d::Vector2i(-1, -1);

    // notify the event to the application window
    if (self.sfWindow)
    {
        sf3d::Event event;
        event.type = sf3d::Event::TouchEnded;
        event.touch.finger = index;
...
////////////////////////////////////////////////////////////
- (void)notifyCharacter:(sf3d::Uint32)character
{
    if (self.sfWindow)
    {
        sf3d::Event event;
        event.type = sf3d::Event::TextEntered;
