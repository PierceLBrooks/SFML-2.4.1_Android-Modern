...
namespace
{
    ...
    bool isJoystick(udev_device* udevDevice)
    {
        ...
        // SFML3D doesn't support evdev yet, so make sure we only handle /js nodes
    ...
    void updatePluggedList(udev_device* udevDevice = NULL)
    {
                ...
                if (isJoystick(udevDevice))
                {
                    ...
                    if (record == joystickList.end())
                    {
                        ...
                        else if (std::strstr(action, "remove"))
                        {
                            ...
                            sf3d::err() << "Trying to disconnect joystick that wasn't connected" << std::endl;
                        }
                    }
                }
        ...
        if (!udevEnumerator)
        {
            sf3d::err() << "Error while creating udev enumerator" << std::endl;
            return;
        }
        ...
        if (result < 0)
        {
            sf3d::err() << "Error while adding udev enumerator match" << std::endl;
            return;
        }
        ...
        if (result < 0)
        {
            sf3d::err() << "Error while enumerating udev devices" << std::endl;
            return;
        }
    ...
    unsigned int getJoystickVendorId(unsigned int index)
    {
        if (!udevContext)
        {
            sf3d::err() << "Failed to get vendor ID of joystick " << joystickList[index].deviceNode << std::endl;
            return 0;
        }
    ...
    unsigned int getJoystickProductId(unsigned int index)
    {
        if (!udevContext)
        {
            sf3d::err() << "Failed to get product ID of joystick " << joystickList[index].deviceNode << std::endl;
            return 0;
        }
    ...
    std::string getJoystickName(unsigned int index)
    {
        ...
        sf3d::err() << "Unable to get name for joystick " << devnode << std::endl;
...
void JoystickImpl::initialize()
{
    ...
    if (!udevContext)
    {
        sf3d::err() << "Failed to create udev context, joystick support not available" << std::endl;
        return;
    }
