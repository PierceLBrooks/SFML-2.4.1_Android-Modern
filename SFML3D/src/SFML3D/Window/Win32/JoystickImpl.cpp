...
namespace
{
    ...
    const sf3d::Time connectionRefreshDelay = sf3d::milliseconds(500);
    ConnectionCache connectionCache[sf3d::Joystick::Count];
    ...
    std::string getErrorString(DWORD error)
    {
        ...
        sf3d::String message = buffer;
        LocalFree(buffer);
        return message.toAnsiString();
    }

    // Get the joystick's name
    sf3d::String getDeviceName(unsigned int index, JOYCAPS caps)
    {
        // Give the joystick a default name
        sf3d::String joystickDescription = "Unknown Joystick";
        ...
        if (result != ERROR_SUCCESS)
        {
            ...
            if (result != ERROR_SUCCESS)
            {
                sf3d::err() << "Unable to open registry for joystick at index " << index << ": " << getErrorString(result) << std::endl;
        ...
        if (result != ERROR_SUCCESS)
        {
            sf3d::err() << "Unable to query registry key for joystick at index " << index << ": " << getErrorString(result) << std::endl;
            return joystickDescription;
        }
        ...
        if (result != ERROR_SUCCESS)
        {
            sf3d::err() << "Unable to open registry key for joystick at index " << index << ": " << getErrorString(result) << std::endl;
            return joystickDescription;
        }
        ...
        if (result != ERROR_SUCCESS)
        {
            sf3d::err() << "Unable to query name for joystick at index " << index << ": " << getErrorString(result) << std::endl;
            return joystickDescription;
        }
