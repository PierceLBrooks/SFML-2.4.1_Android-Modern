...
-(id)initWithView:(NSView *)view
{
    if ((self = [super init]))
    {
        ...
        if (m_view == nil)
        {
            sf3d::err() << "No view was given to initWithWindow:." << std::endl;
            return self;
        }
...
-(void)setWindowPositionToX:(int)x Y:(int)y
{
    (void)x;
    (void)y;
    sf3d::err() << "Cannot move SFML3D area when SFML3D is integrated in a NSView. Use the view handler directly instead." << std::endl;
}
