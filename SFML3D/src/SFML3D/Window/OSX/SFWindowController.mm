...
#import <SFML3D/Window/OSX/NSImage+raw.h>
#import <SFML3D/Window/OSX/Scaling.h>
...
-(id)initWithWindow:(NSWindow*)window
{
    if ((self = [super init]))
    {
        ...
        if (m_window == nil)
        {
            sf3d::err() << "No window was given to -[SFWindowController initWithWindow:]." << std::endl;
            return self;
        }
        ...
        if (m_oglView == nil)
        {
            sf3d::err() << "Could not create an instance of NSOpenGLView "
...
-(id)initWithMode:(const sf3d::VideoMode&)mode andStyle:(unsigned long)style
{
    ...
    if ([NSThread currentThread] != [NSThread mainThread])
    {
         ...
         */
        sf3d::err() << "Cannot create a window from a worker thread. (OS X limitation)" << std::endl;
    ...
    if ((self = [super init]))
    {
        ...
        m_oglView = nil;
        m_requester = 0;
        m_fullscreen = (style & sf3d::Style::Fullscreen);
        m_restoreResize = NO;
...
-(void)setupFullscreenViewWithMode:(const sf3d::VideoMode&)mode
{
    ...
    if (m_window == nil)
    {
        sf3d::err() << "Could not create an instance of NSWindow "
    ...
    if (masterView == nil)
    {
        sf3d::err() << "Could not create an instance of SFBlackView "
                  << "in -[SFWindowController setupFullscreenViewWithMode:]."
                  << std::endl;
        return;
    }
...
////////////////////////////////////////////////////////
-(void)setupWindowWithMode:(const sf3d::VideoMode&)mode andStyle:(unsigned long)style
{
    // We know that style & sf3d::Style::Fullscreen is false.
...
-(void)setWindowPositionToX:(int)x Y:(int)y
{
    ...
    // Flip for SFML3D window coordinate system and take titlebar into account
...
-(void)processEvent
{
    ...
    if ([NSThread currentThread] != [NSThread mainThread])
    {
         ...
         * for more information.
         */
        sf3d::err() << "Cannot fetch event from a worker thread. (OS X restriction)" << std::endl;
