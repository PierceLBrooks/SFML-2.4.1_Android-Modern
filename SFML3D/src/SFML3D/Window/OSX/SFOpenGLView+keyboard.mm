...
#include <SFML3D/Window/OSX/HIDInputManager.hpp> // For localizedKeys and nonLocalizedKeys

#import <SFML3D/Window/OSX/SFKeyboardModifiersHelper.h>
#import <SFML3D/Window/OSX/SFOpenGLView.h>
#import <SFML3D/Window/OSX/SFOpenGLView+keyboard_priv.h>
...
{
    // Transmit to non-SFML3D responder
    ...
    {
        sf3d::Event::KeyEvent key = [SFOpenGLView convertNSKeyEventToSFML3DEvent:theEvent];

        if (key.code != sf3d::Keyboard::Unknown) // The key is recognized.
    ...
    if (m_useKeyRepeat || ![theEvent isARepeat])
    {
        ...
        else
        {
            ...
            // Send each character to SFML3D event requester
...
////////////////////////////////////////////////////////
+(sf3d::Event::KeyEvent)convertNSKeyEventToSFML3DEvent:(NSEvent*)event
{
    // Key code
    sf3d::Keyboard::Key key = sf3d::Keyboard::Unknown;

    // First we look if the key down is from a list of characters
    // that depend on keyboard localization.
    NSString* string = [event charactersIgnoringModifiers];
    if ([string length] > 0)
        key = sf3d::priv::HIDInputManager::localizedKeys([string characterAtIndex:0]);

    // If the key is not a localized one, we try to find a corresponding code
    // through virtual key code.
    if (key == sf3d::Keyboard::Unknown)
        key = sf3d::priv::HIDInputManager::nonLocalizedKeys([event keyCode]);
