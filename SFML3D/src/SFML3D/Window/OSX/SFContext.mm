                              ...
                              const ContextSettings& settings)
{
    ...
    // SFML3D 2 Graphics module uses some OpenGL features that are deprecated in
    ...
    else
    {
        ...
        {
            sf3d::err() << "Warning. Compatibility profile not supported on this platform." << std::endl;
            m_settings.attributeFlags |= ContextSettings::Core;
        }
    ...
    {
        sf3d::err() << "Warning. OpenGL debugging not supported on this platform." << std::endl;
    ...
    if (pixFmt == nil)
    {
        sf3d::err() << "Error. Unable to find a suitable pixel format." << std::endl;
        return;
    }
    ...
    if (sharedContext != nil)
    {
        ...
        {
            sf3d::err() << "Failed to deactivate shared context before sharing" << std::endl;
            return;
        }
    }
    ...
    if (m_context == nil)
    {
        sf3d::err() << "Error. Unable to create the context. Retrying without shared context." << std::endl;
        m_context = [[NSOpenGLContext alloc] initWithFormat:pixFmt
                                             shareContext:nil];

        if (m_context == nil)
            sf3d::err() << "Error. Unable to create the context." << std::endl;
        else
            sf3d::err() << "Warning. New context created without shared context." << std::endl;
    }
